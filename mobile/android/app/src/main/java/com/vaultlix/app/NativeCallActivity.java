...
public class NativeCallActivity extends Activity implements NativeWebRtcCallEngine.Listener {
    ...
    private void buildUi(String caller) {
        ...
        TextView name = label(caller == null || caller.trim().isEmpty() ? "Private call" : caller.trim(), 34, Color.WHITE);
        ...
        status = label("Connecting securely…", 21, Color.rgb(248,241,234)); root.addView(status);
        TextView secure = label("End-to-end encrypted · relayed privately", 14, Color.rgb(180,170,176));
        ...
        Button mute = button("Mute"); mute.setOnClickListener(v -> { muted=!muted; engine.setMuted(muted); mute.setText(muted?"Unmute":"Mute"); });
        Button route = button("Speaker"); route.setOnClickListener(v -> { speaker=!speaker; configureAudio(speaker); route.setText(speaker?"Phone":"Speaker"); });
        Button end = button("End"); end.setTextColor(Color.WHITE); end.setBackgroundColor(Color.rgb(185,79,99));
    ...
    @Override public void onState(String value) { runOnUiThread(() -> { if (connectedAt == 0 && status != null) status.setText("Connecting securely…"); }); }
    ...
    private void finishCall() {
        ...
        String history = connectedAt == 0 ? "" : "Encrypted call · " + formatDuration((System.currentTimeMillis()-connectedAt)/1000);
