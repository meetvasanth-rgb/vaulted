...
test('standard numbers use expiring PostgreSQL reservations while Reserve allocation remains closed', () => {
  ...
  assert.doesNotMatch(client, /Early Tester Reward|Four zeros|Number sequence|Repeated digits|Repeated pairs/);
  assert.match(server, /Reserve allocation is deliberately absent from this public flow/);
});
